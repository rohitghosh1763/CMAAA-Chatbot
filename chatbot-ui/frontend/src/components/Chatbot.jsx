...
function Chatbot({ onClose }) { 
    ...
    // MODIFIED: Return only the chat window structure. Removed the outer full-screen div.
    // Added font-sans here.
    return (
        <div className="w-full max-w-md bg-white shadow-xl rounded-lg flex flex-col h-[70vh] md:h-[85vh] font-sans"> {/* Adjusted height for responsiveness */}
            <header className="bg-blue-600 text-white p-4 rounded-t-lg flex justify-between items-center">
                <h1 className="text-xl font-semibold text-center">
                    Rasa ChatBot
                </h1>
                {/* Optional: Close button inside chat header */}
                {onClose && (
                     <button 
                        onClick={onClose} 
...
//     // Use a ref to track if we've shown the welcome message
//     // This avoids re-renders and dependency issues with useEffect
//     const welcomeMessageShown = useRef(false);
...
//       // Only show welcome message if we haven't shown it yet
//       if (!welcomeMessageShown.current) {
//         welcomeMessageShown.current = true;
...
//         // Add the random welcome message
//         setTimeout(() => {
//           const welcomeMessage = {
//             text: randomWelcomeMessage,
//             sender: "bot",
